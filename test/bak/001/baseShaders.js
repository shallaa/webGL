...
bsGL.shaderSource = {
    color: {
        ...
        shader: {
            ...
            '}',
            fStr: '',
            vMain: ' gl_Position = uPixelMatrix*positionMTX(uPosition)*rotationMTX(uRotation)*scaleMTX(uScale)*vec4(aVertexPosition, 1.0);\n',
            fMain: ' gl_FragColor = vec4(vColor[0],vColor[1],vColor[2], vColor[3]);\n'
        },
        render: '' +
        ' gl.uniform4fv(tProgram.uColor, tMaterial.color) \n' +
        ' gl.drawArrays(gl.TRIANGLE_STRIP, 0, tVBO.num); \n'
    },
    bitmap: {
        attribute: [
            ['vec3', 'aVertexPosition'],
